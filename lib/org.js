...
export const getOrganization = cache(async () => {
    ...
    let slug = (await headers()).get('x-organization-slug') || '';
    ...
    if (!slug) {
        ...
        if (parts.length >= 3) {
            ...
            // Do not return null yet, allow further check
        }
    }
    }
    ...
    try {
        ...
        if (!data) {
            ...
            // If the slug was explicitly provided (subdomain), we should 404.
            // If we are on the root domain, slug is null/empty, we return null for landing page.
